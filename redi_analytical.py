def redi_analytical(ds,kappa,chanloc,v,vpac,modelh,ntime,dt):
    ...
    for time in range(1, ntime):
        ...
        Cpacnew[1:-1]=(Cpacold[1:-1]+
                   timestep*((kappa[2:]*pacconst2[2:]*Cpacold[2:]*(modelh[1:-1]+modelh[2:])/modelh[2:]/2
                            ...
                            -wpac[1:]*Cpacold[1:-1]/modelh[1:-1]+kappav*apac*np.exp(wpac[72]*(-4000+modelh[1:-1])/kappav)))#
        Cnew[0]=0
        Cnew[-1]=modelh[-1]
        Cpacnew[-1]=Cpacnew[-2]
        Cold=Cnew
...
def redi_analytical_AABW(ds,kappa,chanloc,v,vpac,modelh,ntime):
    from scipy import signal
    import numpy as np
    #ntime=1080000
    ny=144
    ...
    for time in range(1, ntime):
        ...
        Cpacnew[1:-1]=(Cpacold[1:-1]+
                   timestep*((kappa[2:]*pacconst2[2:]*Cpacold[2:]*(modelh[1:-1]+modelh[2:])/modelh[2:]/2
                              ...
                              kappa[1:-1]*Cpacold[1:-1]*(modelh[1:-1]+modelh[0:-2])/modelh[1:-1]/2
                              +kappa[1:-1]*Cpacold[0:-2]*(modelh[1:-1]+modelh[0:-2])/modelh[0:-2]/2)/dy**2
                            -(pacconst2[2:]*vpac[2:]*(Cpacold[2:]+Cpacold[1:-1])/(modelh[2:]+modelh[1:-1])-vpac[1:-1]*
        ...
        Cpacnewll[1:maxll]=(Cpacoldll[1:maxll]+
                   timestep*((kappa[2:maxll+1]*pacconst2[2:maxll+1]*Cpacoldll[2:maxll+1]
                              *(modelhll[1:maxll]+modelhll[2:maxll+1])/modelhll[2:maxll+1]/2
                              -kappa[2:maxll+1]*pacconst2[2:maxll+1]*Cpacoldll[1:maxll]*
                              (modelhll[1:maxll]+modelhll[2:maxll+1])/modelhll[1:maxll]/2-
                              kappa[1:maxll]*Cpacoldll[1:maxll]*(modelhll[1:maxll]+modelhll[0:maxll-1])/modelhll[1:maxll]/2
                              +kappa[1:maxll]*Cpacoldll[0:maxll-1]*
                              (modelhll[1:maxll]+modelhll[0:maxll-1])/modelhll[0:maxll-1]/2)/dy**2
                            -(pacconst2[2:maxll+1]*vpacll[2:maxll+1]*(Cpacoldll[2:maxll+1]+Cpacoldll[1:maxll]
        ...
        aatl=(Cold[1:-1]*(modelh[1:-1]+ll[1:-1])/modelh[1:-1]-Cs[1:-1]*(modelh[1:-1]+ll[1:-1]))*(watl[72]/kappav)/(-modelh[1:-1]-ll[1:-1]+kappav/watl[72]*(
                    np.exp(watl[72]*(-4000+modelh[1:-1]+ll[1:-1])/kappav)-np.exp(watl[72]*(-4000)/kappav)))
        apac=Cpacold[1:-1]*(modelh[1:-1]+ll[1:-1])/modelh[1:-1]*(wpac[72]/kappav)/(-modelh[1:-1]-ll[1:-1]+kappav/wpac[72]*(
            np.exp(wpac[72]*(-4000+modelh[1:-1]+ll[1:-1])/kappav)- np.exp(wpac[72]*(-4000)/kappav)))
        if time==ntime-1:
            C=Cnew
            Cpac=Cpacnew
            Cll=Cnewll
